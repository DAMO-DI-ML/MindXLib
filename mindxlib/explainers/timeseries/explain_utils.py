...
class PatchAttributionTorch():
    ...
    """
    def __init__(self, func, patch_size, x_size, is_numpy_model, generator, device,
    ...
    def model_predict_w_replace(self, x, cared_fid, replace_x):
        ...
        if len(result.shape) != 2:
            raise ValueError(f"Result dimension must be two-dimensional, i.e., [num_samples, model_output_dim]. Current shape: {result.shape}")

        return result

    def compute_interactions(self, x, cared_fid):
        ...
        for ii in range(int(self.patch_num)):
            ...
            margs_ii = self.model_predict_w_replace(x, cared_fid, mixed_ii).mean(dim=0)
            ...
            for jj in range(ii + 2, min(ii + KK, int(self.patch_num))):
                ...
                margs_03 = self.model_predict_w_replace(x, cared_fid, mixed_ij_03).mean(dim=0)
                ...
                margs_12 = self.model_predict_w_replace(x, cared_fid, mixed_ij_12).mean(dim=0)
    ...
    def attribute(self, x, cared_fid, lambda_1=None):
        ...
        with torch.no_grad():
            self.compute_interactions(x, cared_fid)
